...
serve:
	...
	docker run -p $(PORT):$(PORT) --volume="$(PROJ_ROOT):/srv/jekyll" -e "JEKYLL_ENV=local" -it $(DOCKER_IMG) jekyll serve --watch --incremental --drafts --port $(PORT) --config _config.yml,_config.docker.yml
